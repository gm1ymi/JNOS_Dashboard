...
function parseMailLogLine(line) {
    ...
    m = line.match(/^(.+?) deliver Msg-Id: (\S+) To: (\S+) From: (\S+) Subject: (.+)$/);
    if (m) {
        return {
            type: 'deliver',
            timestamp: formatMailTimestamp(m[1]),
...
//
// Two things ARE parsed out of it: connection version, and disconnect.
// AX.25 2.2 negotiates extended (mod-128) operation with a SABME frame;
// legacy AX.25 (v1/v2.0) uses plain SABM (mod-8). A DISC frame marks a
// disconnect. Lines look like:
//   "AX25: K1YMI->LFALLS SABME(P)"
//   "AX25: K1YMI->LFALLS DISC(P)"
// Shows the most recently detected connection's version (or 'none' once
// disconnected), same "current state" pattern as MBox Users.
let traceTail = null;
let lastAx25Version = 'none'; // 'none' until a connection is seen, or after a disconnect

function checkAx25Version(line) {
    const setupMatch = line.match(/AX25: \S+->\S+ (SABME?)\(P\)/);
    if (setupMatch) {
        lastAx25Version = setupMatch[1] === 'SABME' ? 'vr2.2' : 'vr1';
        io.emit('ax25_version_update', lastAx25Version);
        return;
    }

    const discMatch = line.match(/AX25: \S+->\S+ DISC(\(P\))?/);
    if (discMatch) {
        lastAx25Version = 'none';
        io.emit('ax25_version_update', lastAx25Version);
    }
}
...
io.on('connection', (socket) => {
    ...
    socket.emit('ax25_version_update', lastAx25Version);
});
